...
use Duplicator\Installer\Core\Deploy\DbUserMode;
use Duplicator\Installer\Utils\Log\Log;
use Duplicator\Installer\Core\Params\PrmMng;
use Duplicator\Libs\Snap\SnapJsonSerialize;
...
class DUPX_DBInstall
{
    ...
    protected function beforeInstallDatabaseActions()
    {
        ...
        DbUserMode::removeAllUserMetaKeysOfCurrentPrefix();
    ...
    {
        $json = SnapJsonSerialize::serializeObj($this, array('dbh'));
    ...
    protected function loadData()
    {
        ...
        SnapJsonSerialize::unserializeToObject($json, $this);
    ...
    protected function dbActionRename()
    {
        ...
        DbUserMode::copyRenamedTableToOrigPrefix();
    }
    ...
    protected function runCleanupRoutines()
    {
    ...
    private static function applyQueryFixes(&$query)
    {
        ...
        if (is_null($replaceRules)) {
            ...
            $replaceRules = self::mergeSearchReplaceRules($replaceRules, self::rulesMySQLEngine());
    ...
    private static function rulesTableNames()
    {
        ...
        foreach ($mapping as $oldPrefix => $newMapping) {
            ...
            foreach ($newMapping as $newPrefix => $commons) {
                Log::info('CHANGE TABLES PREFIX ' . $oldPrefix . ' TO ' . $newPrefix . 'FOR ' . Log::v2str($commons));
                ...
                for ($i = 0; $i < ceil(count($quoteCommons) / self::TABLES_REGEX_CHUNK_SIZE); $i++) {
                    $subArray = array_slice($quoteCommons, $i * self::TABLES_REGEX_CHUNK_SIZE, self::TABLES_REGEX_CHUNK_SIZE);

                    if (count($subArray) == 0) {
                        break;
                    }

                    $result['search'][]  = '/' . $quoteOldPrefix . '(' . implode('|', $subArray) . ')/m';
     ...
     * return search and replace rules
     *
     * @return array return search and replace list
     * @throws Exception
     */
    private static function rulesMySQLEngine()
    {
        $result = array(
            'search'  => array(),
            'replace' => array()
        ...
        if (!empty($invalidEngines)) {
            $result['search'][]  = '/^(\s*(?:\/\*!\d+\s)?\s*CREATE.+ENGINE=)(' . implode('|', $invalidEngines) . ')(.*)$/ms';
            $result['replace'][] = '$1' . DUPX_DB_Functions::getInstance()->getDefaultEngine() . '$3';
        }
