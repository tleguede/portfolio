...
class DUPX_DB_Table_item
{
    ...
    {
        if ($this->isUserTable()) {
            return $this->usersTablesExtractCheck();
        }
    ...
    {
        if (ParamDescUsers::getUsersMode() !== ParamDescUsers::USER_MODE_IMPORT_USERS) {
            return true;
        }
     ...
     * @return boolean
     */
    protected function usersTablesExtractCheck()
    {
        switch (ParamDescUsers::getUsersMode()) {
            case ParamDescUsers::USER_MODE_KEEP_USERS:
                return false;
            case ParamDescUsers::USER_MODE_OVERWRITE:
