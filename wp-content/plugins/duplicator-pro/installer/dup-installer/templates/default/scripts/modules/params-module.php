...
<script>
    ...
    const userSubsiteModeWrapper = <?php echo SnapJson::jsonEncode($paramsManager->getFormWrapperId(PrmMng::PARAM_ADD_SUBSITE_USER_MODE)); ?>;
    ...
    $(document).ready(function() {
        ...
        $('#' + installTypeInputWrapper + ' input[type=radio]').change(function() {
            ...
            switch (parseInt(selectedVal)) {
                ...
                case <?php echo DUPX_InstallerState::INSTALL_MULTISITE_SUBFOLDER; ?>:
                    ...
                    $('#' + userSubsiteModeWrapper + 'input').prop('disabled', true);
                    $('#' + userSubsiteModeWrapper).addClass('param-wrapper-disabled');
                ...
                case <?php echo DUPX_InstallerState::INSTALL_SINGLE_SITE_ON_SUBFOLDER; ?>:
                    ...
                    $('#' + userSubsiteModeWrapper + ' input').prop('disabled', false);
                    $('#' + userSubsiteModeWrapper).removeClass('param-wrapper-disabled');
                    $('#' + tablePrefixInputId).prop('disabled', true);
        ...
        $('#' + userModeWrapper + ' input[type=radio]').change(function() {
            let selectedVal = $(this).val();
            switch (selectedVal) {
                case <?php echo json_encode(ParamDescUsers::USER_MODE_KEEP_USERS); ?>:
                    $('#' + keepUsersInputId).prop('disabled', false);
                    $('#' + keepUsersWrapper).removeClass('param-wrapper-disabled').addClass('param-wrapper-enabled');
                    break;
                case <?php echo json_encode(ParamDescUsers::USER_MODE_OVERWRITE); ?>:
                case <?php echo json_encode(ParamDescUsers::USER_MODE_IMPORT_USERS); ?>:
        ...
        $('#' + userSubsiteModeWrapper + ' input[type=radio]').change(function() {
            let selectedVal = $(this).val();
            switch (selectedVal) {
                case <?php echo json_encode(ParamDescUsers::USER_MODE_KEEP_USERS); ?>:
