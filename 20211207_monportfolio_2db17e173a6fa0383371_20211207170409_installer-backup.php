...
class DUPX_Bootstrap
{
    ...
    const ARCHIVE_FILENAME   = '20211207_monportfolio_2db17e173a6fa0383371_20211207170409_archive.zip';
    const ARCHIVE_SIZE       = '35014486';
    const INSTALLER_DIR_NAME = 'dup-installer';
    const PACKAGE_HASH       = '2db17e1-07170409';
    const SECONDARY_PACKAGE_HASH = 'd4e3f3b-07170409';
    ...
    */
    ...
    public function run()
    {
        ...
        $this->log('==DUPLICATOR INSTALLER BOOTSTRAP v4.0.5.3==');
        $this->log('----------------------------------------------------');
        ...
        if (!$manual_extract_found) {
            ...
            if (($this->archiveRatio < 90) && ($this->archiveActualSize > 0) && ($this->archiveExpectedSize > 0)) {
                ...
                $error       = "<b>Archive file size warning.</b><br/> The expected archive size is <b class='pass'>[{$archiveExpectedEasy}]</b>. "
                    ...
                    . "<ul>"
                    . "<li>Download the whole archive from the source website (open WP Admin &gt; Duplicator Pro &gt; Packages) and validate that the file size is close to the expected size. </li>"
